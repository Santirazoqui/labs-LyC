...
welcome :: IO ()
welcome = do putStrLn $ "This is the test program for Programming Languages Lab 1"
...
runHappy :: IO [String]
runHappy = do (out,err) <- runCommandNoFail "happy -i" "ParCPP.y"
              return $ grep "conflict" err

writeDriver :: IO ()
writeDriver = writeFile "testdriver.hs" driver
...
parseArgs _ = do hPutStrLn stderr "Usage: progs-test-lab1 file.cf"
                 exitFailure
